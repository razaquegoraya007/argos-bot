...
async def start_monitor(ctx, towns, products, stores):
    ...
    async def monitor_product(product, town):
        try:
            ...
            if not stores:
                ...
                save_data(data)
                # Product is not available in this town
                return


            print(f"Found {len(stores)} stores for {product_name} in {town}")
            # check if product was available in this town before
            try:
                if not data['products'][product]['availability'][town]:
            ...
            print(f"Error monitoring product {product} in {town}: {str(e)}")
            return None
    ...
    async def send_product_notifications(ctx, available_products, town):
        ...
        for product, stores in available_products:
            ...
            # message.add_field(name="Product Price", value=product_data['price'])
            # message.add_field(name="Product URL", value=product_data['product_url'])
            ...
            for store in stores:
                ...
                # message.add_field(name="Availability", value='In-Stock' if store['isAvailable'] else 'Out of Stock')
                # message.add_field(name="Store Id", value=store['id_'])
        ...
        for town in towns:
            ...
            for result in results:
                if result:
                    ...
                    if not data['products'][product]['isAvailable']:
                        data['products'][product]['isAvailable'] = True
                        available_products.append((product, stores))

        # if available_products:
        #     await send_product_notifications(ctx, available_products)
    ...
    while True:
        ...
        data = load_data()  # Reload data for the next iteration
